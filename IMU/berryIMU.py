...
accyoffset = -213
acczoffset = 8514
g = math.sqrt(accxoffset^2+accyoffset^2+accyoffset^2)
...
while True:
    ...
    pitch = ACCx/(4*g)#math.asin(ACCx/(4*g))*RAD_TO_DEG
    roll = math.atan2(ACCy,ACCz)*RAD_TO_DEG
    ...
    if 1:                       #Change to '0' to stop showing the angles from the accelerometer
        outputString += "# ACCX/Y/Z Raw %5.2f/%5.2f/%5.2f " % (ACCx, ACCy, ACCz)
        outputString += "# ACCX/Y angles %5.2f/%5.2f " % (AccXangle, AccYangle)
        outputString += "# Pitch/Roll %5.2f/%5.2f " % (pitch, roll)
