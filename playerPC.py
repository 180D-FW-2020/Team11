...
FULLSCREEN = False  # I use this to debug
if FULLSCREEN:
...
class PlayerPC:
	...
	def button(self, text, x, y, w, h, click, inactive_colour=BLUE, active_colour=LIGHT_BLUE, text_colour=WHITE):
		...
		text_surf, text_rect = self.text_objects(text, SMALL_TEXT, colour=text_colour)
	...
	def toggle_btn(self, text, x, y, w, h, click, text_colour=BLACK, enabled=True, draw_toggle=True, blit_text=True,
		...
		if enabled and draw_toggle:
			...
			self.draw_circle(SCREEN, int(x + TOGGLE_WIDTH), y + h // 4, h // 4, enabled_color)
			self.draw_circle(SCREEN, int(x + TOGGLE_WIDTH + TOGGLE_ADJ), y + h // 4, h // 4, enabled_color)
			self.draw_circle(SCREEN, int(x + TOGGLE_WIDTH + TOGGLE_ADJ), y + h // 4, h // 5, WHITE)  # small inner circle
		elif draw_toggle:
			pygame.draw.rect(SCREEN, WHITE, (x + TOGGLE_WIDTH - h // 4, y, TOGGLE_ADJ + h, rect_height))
			pygame.draw.rect(SCREEN, disabled_color, (x + TOGGLE_WIDTH, y, TOGGLE_ADJ, rect_height))
			self.draw_circle(SCREEN, int(x + TOGGLE_WIDTH), y + h // 4, h // 4, disabled_color)
			self.draw_circle(SCREEN, int(x + TOGGLE_WIDTH + TOGGLE_ADJ), y + h // 4, h // 4, disabled_color)
			self.draw_circle(SCREEN, int(x + TOGGLE_WIDTH), y + h // 4, h // 5, WHITE)  # small inner circle
		if blit_text:
			text_surf, text_rect = self.text_objects(text, MEDIUM_TEXT, colour=text_colour)
	...
	def settings(self):
		if useCV:
			...
			try:
				SCREEN.fill(WHITE)
				text_surf, text_rect = self.text_objects('Settings', MENU_TEXT)
				...
				print("An error occurred getting settings 1", flush=True)
				traceback.print_exc()
